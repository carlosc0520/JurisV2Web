-- Optimized fragment for `ELSE` branch of JURIS.USP_CRUD_ENTRIES_BUSQUEDA
-- Starts at: END ELSE BEGIN ... 0. NORMALIZAR PARÁMETROS
-- Replaces logic from normalization onward with materialization and safer predicates
...
-- Esto evita llamar a funciones por fila repetidas veces en el CTE
...
SELECT
  ...
  CAST(dbo.RemoveNonAlphanumeric(ISNULL(e.SHORTSUMMARY, '')) AS VARCHAR(500)) COLLATE Latin1_General_CI_AI AS SHORTSUMMARY_N,
    ...
    AND (
      ...
      OR (
        @MODO = '3' AND (
          ...
          OR CHARINDEX(CAST(dbo.RemoveNonAlphanumeric(@GLOBAL) AS VARCHAR(500)), A.SHORTSUMMARY_N) > 0
        )
        OR (@MODO = '2' AND EXISTS (
          ...
          WHERE CHARINDEX(p.PAL, A.TITULO_N) > 0
            ...
            OR CHARINDEX(p.PAL, A.SHORTSUMMARY_N) > 0
        ...
        OR (@MODO = '1' AND (
          ...
          NOT EXISTS (
            ...
            WHERE CHARINDEX(p.PAL, A.TITULO_N) = 0
              ...
              AND CHARINDEX(p.PAL, A.SHORTSUMMARY_N) = 0
...
-- End of optimized ELSE fragment
